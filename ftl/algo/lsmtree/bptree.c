...
Entry *make_entry(KEYT key,KEYT end,KEYT pbn, bloomfilter *filter){
	Entry *res=(Entry*)malloc(sizeof(Entry));
	res->key=key;
	res->pbn=pbn; res->filter=filter;
	res->end=end;
	res->parent=NULL;
	return res;
}
Entry *level_entry_copy(Entry *input){
	...
	res->filter=malloc(sizeof(bloomfilter));
	bloomfilter_init(res->filter);
	memcpy(res->filter->bit_vector,input->filter->bit_vector,FILTERBIT);
	res->parent=NULL;
}
void *free_entry(Entry *entry){
	if(entry->filter!=NULL)
		bloomfilter_free(entry->filter);
	free(entry);
}
...
level *level_init(level* input, int size){
	...
	input->version=0;
	input->m_size=size;
	input->q=(Queue*)malloc(sizeof(Queue));
	input->q=queue_init(input->q,size);
	return input;
}
...
bpIterator *level_find(level *lev, KEYT key){
	Node *start=level_find_leafnode(lev,0);
	Entry *temp=start->children[0].entry;
	bpIterator *res=malloc(sizeof(bpIterator));
	heap_init(res,lev->m_size);
	int cnt=1;
	while(temp->key <=key){
		if(temp->end>=key)
			heap_push(res,temp);
		if(cnt<start->count){
			temp=start->children[cnt++].entry;
		}
		else{
			start=start->children[MAXC].node;
			if(start==NULL) break;
			temp=start->children[0].entry;
			cnt=1;
		}
	}
	return res;
...
Entry *bp_getNext(bpIterator *iter){
	Entry *res=heap_pop(iter);
	if(res==NULL){
		heap_free(iter);
		return NULL;
	}
...
Node * level_delete(level *lev, Entry *input ){
	KEYT key=input->key;
	Node *temp=level_find_leafnode(lev,key);
	int idx=0;
	for(int i=0; i<temp->count; i++){
		if(temp->children[i].entry->key==key && temp->children[i].entry->version==input->version){
...
void level_free(level *lev){
	Entry *temp;
	while((temp=dequeue(lev->q))!=NULL){
		level_delete(lev,temp);
	}
	queue_free(lev->q);
	free(lev->root);
	free(lev);
...
int main(){
	...
	for(int i=0; i<1000; i++){
		level_insert(lev,make_entry(i,i,i,NULL));
	}
	bpIterator *iter=level_find(lev,100);
	Entry *res;
	while((res=bp_getNext(iter))!=NULL){
		printf("k -%d : v -%d\n",res->key,res->version);
	}
	level_free(lev);
}
